...
cornseqinfo <- makeTxDbFromGFF("~/database/dap/dap/ZeamaysB73v447.gtf", format=c("gtf"),chrominfo = x)
readRDS("E:/index/maizev4gtfchr.rds")

## Profile of ChIP peaks binding to TSS regions

promoter <- getPromoters(TxDb=cornseqinfo, upstream=5000, downstream=1)

##peak-files

files <- list.files(path="~/database/dap/peak/", pattern="*.rmblacklist.bed", full.names=TRUE, recursive=FALSE)
...
for (file in files) {
  ...
  try( {
    ...
    pdf(paste("~/database/dap/AnnoBar/",fn,".AnnoBar.pdf",sep=""))
    print(plotAnnoBar(peakannotate)) ## 可视化 Pie and Bar plot
    dev.off()
    
    pdf(paste("~/database/dap/Annopie/",fn,".Annopie.pdf",sep=""))
    print(plotAnnoPie(peakannotate)) ## 可视化 Pie and Bar plot
    dev.off()
    
    pdf(paste("~/database/dap/vennpie/",fn,".vennpie.pdf",sep=""), width =13, height=7)
    print(vennpie(peakannotate)) ## 可视化 Pie and Bar plot
    dev.off()
    
    pdf(paste("~/database/dap/upsetplot/",fn,".upsetplot.pdf",sep=""))
    print(upsetplot(peakannotate)) ## 可视化 Pie and Bar plot
    dev.off()
    
    pdf(paste("~/database/dap/Distrib/",fn,".Distrib.pdf",sep=""))
    print(plotDistToTSS(peakannotate, title="Distribution of binding loci relative to TSS"))
    dev.off()
    
    ## 输出annotate 文件
    save(peakannotate,file=(paste("~/database/dap/txt/rda/",fn,".rda",sep=""))) # Output peakAnnolist file
    
    write.table(as.data.frame(peakannotate),file=paste("~/database/dap/txt/",fn,".txt",sep=""),sep='\t',quote = F,row.names=FALSE) # Output peakAnnolist file
  })
